...
<head>
    ...
    <title>Pending Business - Admin Dashboard</title>
    ...
    <style>
        * {
        ...
        a.approve:hover { background: #27ae60; }
        a.reject:hover { background: #c0392b; }
...
<body>
    ...
    <div class="main-content">
        ...
        <table>
            ...
            <% 
                List<BusinessOwner> businessOwners = (List<BusinessOwner>)request.getAttribute("businessOwners");
                for(BusinessOwner b: businessOwners ) {
                    if("Pending".equals(b.getBusinessStatus())){
            %>
            <tr>
                <td><%= b.getBusinessName() %></td>
                <td><%= b.getBusinessCategory() %></td>
                <td><%= b.getBusinessAddress() %></td>
                <td><%= b.getBusinessDescription() %></td>
                <td><%= b.getOwnerName() %></td>
                <td><%= b.getOwnerContact() %></td>
                <td><%= b.getBusinessStatus() %></td>
                <td>
                    <a class="action-btn approve" 
                       href="ChangeBusinessStatus?contact=<%= b.getOwnerContact() %>&status=Approved">Approve</a>
                    <a class="action-btn reject" 
                       href="ChangeBusinessStatus?contact=<%= b.getOwnerContact() %>&status=Rejected">Reject</a>
                </td>
            </tr>
            <% } } %>
        </table>
